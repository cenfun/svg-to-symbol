...
const generateSvgSymbolFile = (metadata) => {
    const svgAttrs = Object.keys(metadata.headers).map(k => {
        return `${k}="${metadata.headers[k]}"`;
    }).join(' ');
    const $ = cheerio.load(`<svg ${svgAttrs}></svg>`, {
        xmlMode: true
    });
    const $svg = $('svg');
    metadata.list.forEach(item => {
        const $symbol = $('<symbol></symbol>');
        $symbol.attr('id', metadata.prefix + item.id);
        if (item.viewBox) {
            $symbol.attr('viewBox', item.viewBox);
        }
        $symbol.append(item.content);
        $svg.append($symbol);
    });
    
    const svgSymbol = format($.xml());
...
const outputHandler = (config) => {
    ...
    if (config.outputSvg) {
        ...
        const svgSymbol = generateSvgSymbolFile(config.metadata);
    ...
    if (config.outputCompressed) {
        const compress = require(config.compressLib);
        const compressedStr = [
            `import decompress from '${config.decompressLib}';`,
            `const metadata = '${compress(JSON.stringify(config.metadata))}';`,
            'export default JSON.parse(decompress(metadata));'
        ].join(os.EOL);
        const compressedPath = path.resolve(config.outputDir, config.outputCompressed);
        const compressedSaved = writeFileContentSync(compressedPath, compressedStr);
        if (compressedSaved) {
            logSuccess(`Saved svg compressed js: ${relativePath(compressedPath)}`);
        }
    }
