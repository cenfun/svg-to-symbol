...
const generateSvgList = (config) => {
    ...
    files.forEach(item => {
        ...
        if (config.filenameReg) {
            const filenameReg = new RegExp(config.filenameReg, 'g');
            const nameTest = filenameReg.test(filename);
            //console.log(nameTest);
            if (!nameTest) {
                logError(`ERROR: Filename unmatched with ${filenameReg}: ${filePath}`);
                return;
            }
        }
...
const generateSvgSymbol = (config) => {
    ...
    config.svgList.forEach(item => {
        ...
        let id = path.basename(item.filename, '.svg');
        if (ns) {
            id = `${ns}-${id}`;
        }
        ...
        if (typeof config.onSVGAttribute === 'function') {
            config.onSVGAttribute.call(config, $originalSvg);
        }
